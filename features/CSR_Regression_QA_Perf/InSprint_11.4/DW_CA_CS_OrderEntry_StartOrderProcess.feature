...
Scenario Outline:Verify Account page displays when CSR selects Continue To Verify Account in Locate Account page:[<UsingData>]
	...
	When CSR navigates to Locate Account Page and clicks on Teacher's name
	Then [Verify Account] page should be displayed
	
Examples:
  | UsingData                  							|
  | Perf_QA_CSR_enters_Locate_Account_to_Verify_Account_page	|		
...
Examples:
  | UsingData                  						|
  | Perf_QA_CSR_enters_Verify_Account_to_OrderEntry_page	|		
...
Examples:
  | UsingData                  	|
  | Perf_QA_Order_Entry_tab_highlights	|		
...
Examples:
  | UsingData                  						|
  | Perf_QA_Order_Entry_form_displays_in_Order_Entry_page	|		
...
@Priority2
@5
Scenario Outline:‘Proceed to Checkout’ positioned in the footer region of the ‘Order Entry’ page:[<UsingData>]
...
Examples:
  | UsingData                  								|
  | Perf_QA_Proceed_To_Checkout_button_displays_Order_Entry_page	|		
