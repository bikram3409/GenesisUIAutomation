...
@Priority2
@NotForThisSprint
Scenario Outline:CSR enters the invalid Flyer code in the Flyer Code entry field:[<UsingData>]
	
	Given CSR logs into the application by providing [UserInformation]
	And CSR selects the order type as [Phone Order]
	And CSR navigates to Verify Account Page
	And CSR navigates to Order Entry Page
	When CSR enters the invalid Flyer code
	Then the error message 'Flyer is invalid. No items from this flyer can be added to cart.' should be displayed
	
Examples:
  | UsingData                  								|
  | Invalid_Flyer_Code_entry_CSR_enters_Order_Entry_page	|	
...
@Priority2
@13
Scenario Outline:CSR cannot enter other than alphanumerics in the Flyer Code entry field:[<UsingData>]
	
	Given CSR logs into the application by providing [UserInformation]
	And CSR selects the order type as [Phone Order]
	And CSR navigates to Verify Account Page
	And CSR navigates to Order Entry Page
	When CSR enters special characters in the Flyer code field
	Then the error message '' should be displayed
	
Examples:
  | UsingData                  								|
  | Special_chars_in_Flyer_Code_CSR_enters_Order_Entry_page	|	
