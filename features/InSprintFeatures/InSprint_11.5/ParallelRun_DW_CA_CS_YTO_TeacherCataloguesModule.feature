...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as HST_NL:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as HST_NB:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as HST_NS:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as HST_ON:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as HST_PE:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as GST_AB:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as GST_MB:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as GST_NT:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as GST_NU:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as GST_SK:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
...
@CSR_ReviewCart @Priority1  @DWCACSR-163 
Scenario Outline:Correct subtotals, totals, taxes display for Teacher Catalogues when multiple items are added from multiple flyer for province with applicable taxes as GST_YT:[<UsingData>]
    Given CSR logs into the application by providing [UserInformation]
     And CSR selects the order type as [Phone Order]
     And CSR navigates to Verify Account Page
     And CSR navigates to Order Entry Page
