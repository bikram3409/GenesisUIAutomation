...
Examples:
  | UsingData |
  | CSR_enters_FlyerCode_SpecialCharacter	|			
...
@Priority1
@2
Scenario Outline:CSR sees proper error message on clicking [Proceed to Checkout] when on Teacher order, Item# and Flyer code  is blank and QTY is entered:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_NoFlyerCode_NoItemno_enterQty|
...
@Priority1
@3
Scenario Outline:CSR sees proper error message on clicking [Proceed to Checkout] when on Teacher order,Valid flyer code is entered ,more than one Item# cell is blank and Qty is entered:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_FlyerCode_EntersItemNo_BlankItemNo_enterQty|	
...
@Priority1
@4
Scenario Outline:CSR sees proper error message pop-up on clicking [Proceed to Checkout] when on Teacher order, Item# entered  has one or more speical characters &QTY is entered:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_FlyerCode_ItemNoSpecialCharacte_enterQty|
...
@Priority1
@5
Scenario Outline:CSR sees proper error message pop-up on clicking [Proceed to Checkout] when on Teacher order, Flyer code is left blank,Item# entered is single integer &QTY is entered:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_NoFlyerCode_ItemSerialNo_QTY|
...
@Priority1
@6
Scenario Outline:CSR sees proper error message on entering the Invalid Flyer code in the flyer text box for Teacher Order Module on tabing out:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_FlyerCodeIsNotValidFlyer	|	
...
@Priority1
@7
Scenario Outline:CSR sees proper error message when less than 6 characters are entered in Flyer code text box for Teacher Order Module on tab out:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_FlyerCodeLessThan6Characters	|	
...
@Priority1
@8
Scenario Outline:CSR sees proper error message on entering the Flyer code and tabing out from Flyer code text box for Teacher Order Module. The flyer code entered is  not active for the current month:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_FlyerCodeNotValidforCurrentMonth	|	
...
@Priority2
@9
Scenario Outline:CSR sees proper error message when on Teacher Module item code is entered and flyer code is blank:[<UsingData>]
	
	Given CSR logs into the application by providing [UserInformation]
	And CSR selects the order type as [Phone Order]
	And CSR navigates to Verify Account Page
	And CSR navigates to Order Entry Page
	And CSR enters Item code from active Flyer and enters Paid QTY
	When CSR clicks on Flyer code box and tab out
	Then Error message is displayed for Missing flyer code
Examples:
  | UsingData |
  | CSR_enters_NoFlyer_EnterItemCodeFlyerActive_Qty	|	
...
Examples:
  | UsingData |
  | CSR_enters_ValidItemNo_Qty3Digit|
...
Examples:
  | UsingData |
  | CSR_enters_FlyerCode_ValidItemNo2_InvalidItemNo2_QtyBlank2|
...
#@Priority1
#@13
#Scenario Outline:CSR sees proper error message pop-up on clicking [Proceed to Checkout] when on Teacher order, valid flyer code is entered , multiple rows of Item# is entered with invalid item No and Qty:[<UsingData>]
#	
#	Given CSR logs into the application by providing [UserInformation]
#	And CSR selects the order type as [Phone Order]
#	And CSR navigates to Verify Account Page
...
#Examples:
#  | UsingData |
#  | CSR_enters_FlyerCode_ItemSerialNo_ItemSerialNoSpecialCharacter_ItemSerialNoString|
...
@Priority1
@15
Scenario Outline:CSR sees proper error message pop-up on clicking [Proceed to Checkout] when on Teacher order, valid flyer code is  entered , multiple rows of Item# is entered with invalid item No and Qty. And also rows with valid Item# and qty is present:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_FlyerCode_ValidItemNo2_InvalidItemNo2_Qty|
...
@Priority1
@17
Scenario Outline:CSR sees proper error message pop-up on clicking [Proceed to Checkout] when on Teacher order, flyer code is not entered , multiple rows of Item# is entered with invalid item No and Qty. And also rows with valid Item# and qty is present:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_ValidItemNo2_InvalidItemNo2_Qty|
...
Examples:
  | UsingData |
  | CSR_sees_err_msg_for_Flyer_is_Invalid|
...
@Priority1
@19
Scenario Outline:CSR sees proper error message pop-up on clicking [Proceed to Checkout] when on Teacher order following is present – Row of valid item#,row with Invalid Item#,row with blank Item#:[<UsingData>]
...
Examples:
  | UsingData |
  | CSR_enters_ValidItemNo_InvalidItemNo_ItemNoBlak_Qty|
...
Examples:
  | UsingData |
  | CSR_sees_err_msg_You_must_enter_an_item_Number|  
...
@Priority2
@21
Scenario Outline:CSR is not allowed to enter Non-Numeric value in [Paid Qty] for Teacher Module:[<UsingData>]
	
	Given CSR logs into the application by providing [UserInformation]
	And CSR selects the order type as [Phone Order]
	And CSR navigates to Verify Account Page
	When CSR enters ItemNo and enters Paid QTY as String
	Then System do not allow entry of String value in [Paid Qty]
Examples:
  | UsingData |
  | CSR_enters_ValidItemNo_QtyString|  
...
Examples:
  | UsingData |
  | CSR_sees_err_msg_You_must_enter_a_qty| 
...
Examples:
  | UsingData |
  | CSR_enters_NoFlyerCode_InvalidItemNo_ValidItemNo_QTY_ClickContinue|
...
Examples:
  | UsingData |
  | CSR_enters_NoFlyerCode_InvalidItemNo_ValidItemNo_QTY_ClickCancel|
...
@Priority2
@25
Scenario Outline:CSR is not allowed to enter more than 3 integers in [Paid Qty] for Teacher Module:[<UsingData>]
	
	Given CSR logs into the application by providing [UserInformation]
	And CSR selects the order type as [Phone Order]
	And CSR navigates to Verify Account Page
	When CSR enters ItemNo and enters Paid QTY more than 3 Integers
	Then System do not allow entry of more than 3 integers in [Paid Qty]
Examples:
  | UsingData |
  | CSR_enters_ValidItemNo_QtyFourDigits|
