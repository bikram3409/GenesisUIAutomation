...
@Priority1
@1
Scenario Outline: CSR sees Amount Paid by Credit Card and Amount Paid by Purchase Order labels in the Payment section of Confirm and Submit page:[<UsingData>]
...
And the amount is displayed for credit card as remaining from PO amount
Examples:
  | UsingData 												     |
  | CSR_sees_correct_labels_Payment_Credit_PO_ConfirmSubmit_page |

@ConfirmSubmit_PurchaseOrder
@DWCACSR-2031
@Priority1
@2
Scenario Outline: CSR sees Amount Paid by Purchase Order label in the Payment section of Confirm and Submit page:[<UsingData>]
Given CSR logs into the application by providing [UserInformation]
And CSR selects the order type as [Phone Order]
And CSR navigates to Verify Account Page
And CSR navigates to Order Entry Page
...
And the amount for PO is the same as amount owing
Examples:
  | UsingData 											  |
