Feature: Redemptions & Rewards: BonusFlat
	This feature is to validate the Redemptions & Rewards: BonusFlat scenarios
...
Examples: 
	| UsingData                   			|
	| CSR:TeacherOrder_BonusFlatCoupon_1 	|
...
Examples: 
	| UsingData                   		|
	| CSR:TeacherOrder_BonusFlatCoupon_2|
...
Examples: 
	| UsingData                   		|
	| CSR:TeacherOrder_BonusFlatCoupon_3|
...
Examples: 
	| UsingData                   			|
	| CSR:TeacherOrder_BonusFlatCoupon_4 	|
...
Examples: 
	| UsingData                   			|
	| CSR:TeacherOrder_BonusFlatCoupon_5	|
	

@ORPHAN @Automated @CouponsAndRewards @Manual_BonusFlat @Priority3 
Scenario Outline:CSR can apply Bonus Flat coupon if top-off from DTS makes cart qualify for Bonus Flat coupon:[<UsingData>] 

	Given CSR logs into the application by providing [UserInformation]
	And CSR selects the order type as [Phone Order]
	And CSR navigates to Verify Account Page
	And CSR navigates to Order Entry Page
	And CSR adds Student Clubs items to the cart to meet the minimum amount for Dollars to Spend but not Bonus Flat
	And CSR navigates to Review Cart page from Order Entry page
	And CSR navigates to [Rewards & Coupons] page from [Review Cart] page
	And CSR see the Dollars to Spend promotion
	And CSR clicks on [MAKE SELECTION] button
	And CSR selects items such that the top-off amount makes cart qualify for Bonus Flat coupon
	And CSR clicks on Confirm Selection
	When CSR applies [Bonus Flat coupon]
	Then CSR recieves Bonus Dollars

Examples: 
	| UsingData                    			|
	| CSR:TeacherOrder_BonusFlatCoupon_6 	|
...
Examples: 
	| UsingData             				|
	| CSR:Teacher_BonusFlat_UpsellMessage 	|
