...
Examples: 
	| UsingData                    |
	| CSR:teacher_login_FreeItemCoupon |
...
Examples: 
	| UsingData                                     |
	| CSR:teacher_ManualFreeItem_StudentBonusRedemption |
...
Examples: 
	| UsingData                    |
	| CSR:teacher_login_FreeItemCoupon_2 |
...
Examples: 
	| UsingData                    |
	| CSR:teacher_login_FreeItemCoupon_3 |
...
Examples: 
	| UsingData                    |
	| CSR:teacher_login_FreeItemCoupon_4 |
...
Examples: 
	| UsingData                    |
	| CSR:teacher_FreeItemCoupon_Expires |
...
Examples: 
	| UsingData                                     |
	| CSR:teacher_StudentPaperRedemption_FreeItemCoupon |
...
Examples: 
	| UsingData                                     |
	| CSR:teacher_TeacherBonusRedemption_FreeItemCoupon |
...
Examples: 
	| UsingData                                     |
	| CSR:teacher_TeacherPaperRedemption_FreeItemCoupon |
	

@ORPHAN @Automated @Manual_FreeItem @Priority3 
Scenario Outline:CSR can apply Free item coupon on a cart with top-off amount making the cart qualifying:[<UsingData>] 

	Given CSR logs into the application by providing [UserInformation] 
	And CSR selects the order type as [Phone Order] 
	And CSR navigates to Verify Account Page 
	And CSR navigates to Order Entry Page
	And CSR adds items to cart less than the [minimum amount]for free item and equal to [minimum amount] for Dollars to Spend promotion
	And CSR navigates to Review Cart page from Order Entry page
	And CSR navigates to [Rewards & Coupons] page from [Review Cart] page
	...
	And CSR applies [Free item coupon]
	Then CSR sees Free item displayed in Coupons section

Examples: 
	|UsingData|
	|CSR:Teacher_FreeItem_DTSTopOffAmt_ManualCoupon|
...
Examples: 
	| UsingData                    |
	| CSR:Teacher_ManualFREEItemCoupon |
...
Examples: 
	| UsingData                             |
	| CSR:Teacher_ManualFREEItem_RedeemedCoupon |
