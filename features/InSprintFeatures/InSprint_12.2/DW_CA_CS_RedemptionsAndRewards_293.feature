...
Examples: 
	| UsingData                       |
	| Teacher_DTS_with_minimum_amount_1 |
...
Examples: 
	| UsingData                          |
	| Teacher_DTS_without_minimum_amount |    
...
Examples:
   | UsingData                                    |
   | Teacher_DTS_with_minimum amount_couponamount |
...
Examples:
   | UsingData                       |
   | Teacher_DTS_with_minimum_amount_2 |
...
@5
@Maintenance
Scenario Outline:CSR can redeem Dollars to Spend once per level per customer:[<UsingData>]
	...
	And CSR navigates to Review Cart page from Order Entry page 
	And CSR navigates to [Rewards & Coupons] page from [Review Cart] page
	And CSR selects an item from Dollars to Spend modal 
	And CSR submits the order 
	And CSR navigates to Home page 
	And CSR adds student club items to cart with same [minumum amount] as in previous order 
	When CSR navigates to coupons & rewards page 
	Then CSR does not see the Dollars to spend promotion 
	
Examples:
  | UsingData                       |
  | Teacher_DTS_with_minimum_amount_3 |
...
@6
@Maintenance
Scenario Outline:CSR can redeem Dollars to Spend on second tier if teacher already redeemed on first tier:[<UsingData>]
	...
	And CSR submits the order 
	And CSR navigates to Home page 
	And CSR adds student club items to cart with [tier 2 minumum amount] 
	When CSR navigates to coupons & rewards page 
	Then CSR sees the Dollars to Spend promotion 
Examples:
   | UsingData                       |
   | Teacher_DTS_with_minimum_amount_4 |
